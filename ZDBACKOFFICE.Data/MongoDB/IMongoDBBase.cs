...
namespace ZDBACKOFFICE.Data.MongoDB
{
    public interface IMongoDBBase
    {
        ...
        Task<UpdateResult> UpdateAsync<T>(string collName, T t, string id, bool isObjectId) where T : class, new();
        UpdateResult UpdateManay<T>(string collName, FilterDefinition<T> filter, UpdateDefinition<T> update) where T : class, new();
