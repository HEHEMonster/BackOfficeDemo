...
{
    public partial class MongoDBBase : IMongoDBBase
    {
            ...
            public UpdateResult UpdateManay<T>(string collName, FilterDefinition<T> filter, UpdateDefinition<T> update) where T : class, new()
            {
                try
                {
                    var client = _database.GetCollection<T>(collName);
            ...
            public async Task<bool> AnyAsync<T>(string collName, FilterDefinition<T> filter) where T : class, new()
            {
                ...
                {
                    var client = _database.GetCollection<T>(collName);
                    long count = await client.CountDocumentsAsync(filter);
                    return count > 0;
                }
                catch (Exception ex)
                {
            ...
            public bool Any<T>(string collName, FilterDefinition<T> filter) where T : class, new()
            {
                ...
                {
                    var client = _database.GetCollection<T>(collName);
                    long count = client.CountDocuments(filter);
                    return count > 0;
                }
                catch (Exception ex)
                {
