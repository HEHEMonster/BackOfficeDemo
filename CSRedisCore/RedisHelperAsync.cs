...
partial class RedisHelper {
    ...
    /// <param name="getDataAsync">获取源数据的函数，输入参数是没有缓存的 fields，返回值应该是 (field, value)[]</param>
    /// <returns></returns>
    public static Task<T[]> CacheShellAsync<T>(string key, string[] fields, Func<string[], Task<(string, T)[]>> getDataAsync) => Instance.CacheShellAsync(key, fields, getDataAsync);
